...
1. Trade-offs between mixing and compute time.

1. How to configure samplers in NIMBLE to improve efficiency.

1. Basic checks for convergence of the MCMC.
...
- Denominator is hard to calculate but it is a constant (generally a multivariate integral).
...
- We will refer to the numerator = $[\mbox{data, states, params}]$ as the "model calculations."

Why Markov chain Monte Carlo
=====

- MCMC is a method that avoids integration by sampling proportional to the posterior distribution for both the joint posterior and marginals.

- Markov chain means that each new sample will only depend on the previous sample, and will have properties that should result in drawing samples from the posterior distribution.
...
```{r}
  y <- rpois(100, 3)
  post.sample <- rgamma(10000, 1 + sum(y), 1+100)
  plot(density(post.sample), main ="")
```
...
```{r}
library(mcmcplots)
mcmcplot(martin_alt$nimble_alt$samples)
```
...
p_slice <- c("sigma.proc")
mcmcConf$printSamplers(p_slice, displayControlDefaults = TRUE)
...
coda::effectiveSize(samples)[p_slice]
```


```{r}
## Let's see if we can improve mixing?
mcmcdefs <- list(
  sigma_slice = function(model) {
    mcmcConf <- configureMCMC(model)
    mcmcConf$removeSamplers(c('sigma.proc', 'sigma.obs'))
    mcmcConf$addSampler(target = c('sigma.proc'),
                        type = "slice")
    mcmcConf$addSampler(target = c('sigma.obs'),
                        type = "slice")
    mcmcConf
  },
  sigma_log = function(model){
    mcmcConf <- configureMCMC(model)
    mcmcConf$removeSamplers(c('sigma.proc', 'sigma.obs'))
...
martin_alt_sigma <- compareMCMCs(
              modelInfo = list(model = martin_model_copy), 
              MCMCcontrol = list(niter = 100000,
                     burnin = 10000),
              MCMCs = c("sigma_slice", "sigma_log", "default"),
              nimbleMCMCdefs = mcmcdefs )

make_MCMC_comparison_pages(martin_alt_sigma,
                           dir = "martin_results-user",
                           modelName = "martin-state-space-model-sigma")

library(mcmcplots)
mcmcplot(martin_alt_sigma$default$samples, dir = ".", filename = "statespace_samples_mcmcplot_default")
mcmcplot(martin_alt_sigma$sigma_log$samples, dir = ".", filename = "statespace_samples_mcmcplot_slicesigma")
mcmcplot(martin_alt_sigma$sigma_slice$samples, dir = ".", filename = "statespace_samples_mcmcplot_blocksigma")
```
